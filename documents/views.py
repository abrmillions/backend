...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    def validate_photo(self, request):
        """Validate a professional/profile photo before document creation"""
        try:
            file = request.FILES.get("file")
            if not file:
                return Response({"detail": "No photo provided"}, status=status.HTTP_400_BAD_REQUEST)
            
            # Temporary document object for verification engine (not saved)
            from .models import Document
            temp_doc = Document(
            ...
            # Use "Contractor License" or "Professional License" category
            category = "Contractor License" 
            res = perform_verification([temp_doc], category)
            
            if res and res.get("results") and len(res["results"]) > 0:
                first = res["results"][0]
                # The perform_verification might have updated temp_doc fields
                # We extract the results
                return Response({
                    "status": first.get("status"),
                    "score": first.get("score"),
                    "details": first.get("details") or temp_doc.verification_details,
                    "is_verified": first.get("status") == "verified_true"
                })
            
            return Response({"detail": "Photo validation failed"}, status=status.HTTP_400_BAD_REQUEST)
