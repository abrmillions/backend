...
class LicenseVerificationSerializer(serializers.Serializer):
    ...
    def get_license_photo_url(self, obj):
        ...
        # Use the robust logic from LicenseSerializer
        try:
            # We use the existing LicenseSerializer to get the effective photo URL
            # this ensures we look back at previous licenses, applications, etc.
            ls = LicenseSerializer(context=self.context)
            return ls.get_license_photo_url(obj)
        except Exception:
            # Fallback to current photo if something goes wrong
    ...
    def get_license_photo_base64(self, obj):
        ...
        try:
            ls = LicenseSerializer(context=self.context)
            return ls.get_license_photo_base64(obj)
        except Exception:
            return None
...
class LicenseDownloadView(APIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        try:
            ...
            # Mark as downloaded in data field
            if isinstance(license.data, dict):
                license.data['downloaded'] = True
                license.save(update_fields=['data'])
            elif license.data is None:
                license.data = {'downloaded': True}
                license.save(update_fields=['data'])
