...
class ApplicationAdmin(admin.ModelAdmin):
    ...
    def approve_applications(self, request, queryset):
        ...
        try:
            ...
            verified_only = sys_settings.verified_only_true
        except Exception:
            verified_only = False
        ...
        for app in queryset:
            try:
                if verified_only:
                    ...
                    if not docs.exists():
                        # If no documents, maybe it's okay? Or maybe we should block?
                        # Usually, if verified_only is on, we expect documents.
                        # For now, let's assume if there are no docs, it's not "all verified".
                        skipped += 1
                        continue
                    
                    unverified_docs = docs.filter(is_verified=False)
        ...
        if skipped:
            messages.warning(request, f"Skipped {skipped} application(s) because they have unverified documents (verifiedOnlyTrue is ON).")
    ...
    def save_model(self, request, obj, form, change):
        ...
        # Check for verified_only_true restriction
        if obj.status == "approved" and prev_status != "approved":
            try:
                sys_settings = SystemSettings.get_solo()
                if sys_settings.verified_only_true:
                    unverified_docs = obj.documents.filter(is_verified=False)
                    if unverified_docs.exists() or not obj.documents.exists():
                        messages.error(request, "Cannot approve application: Some documents are not verified (verifiedOnlyTrue is ON).")
                        obj.status = prev_status or "pending"
                        # If it's a new object and we block it, we might want to prevent creation or set to pending
            except Exception:
                pass
