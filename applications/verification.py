...
# ============================================================================
# IMAGE ENHANCEMENT FUNCTIONS (For better OCR and AI Vision quality)
# ============================================================================

def enhance_image_for_ocr(image_bytes: bytes) -> bytes:
    """Enhance image quality for better OCR and AI analysis.
    
    This function applies several preprocessing steps to improve text readability:
    ...
    try:
        ...
        enhanced = enhancer.enhance(1.8)  # Increase contrast by 80%
        ...
        # Save to bytes
        output = io.BytesIO()
        enhanced_rgb.save(output, format='PNG', quality=95)
        return output.getvalue()
    except Exception as e:
        print(f"DEBUG: Image enhancement error: {str(e)}")
        return image_bytes  # Return original on failure


def enhance_and_preprocess_image(image_bytes: bytes, target_dpi: int = 300) -> bytes:
    """Advanced image preprocessing for documents with low quality.
    
    Steps:
    1. Open and convert image
    2. Apply binarization (threshold-based)
    3. Scale up for better resolution
    4. Apply sharpening
    5. Return enhanced bytes
    """
    if not PIL_AVAILABLE:
        return image_bytes
    ...
    try:
        ...
        from PIL import ImageEnhance
        enhanced = ImageEnhance.Contrast(smoothed).enhance(2.0)
        enhanced = ImageEnhance.Sharpness(enhanced).enhance(2.0)
        
        # Scale up if image is too small (common for phone photos)
        ...
        print(f"DEBUG: Advanced preprocessing error: {str(e)}")
        return image_bytes
...
def extract_text_from_image(image_bytes: bytes, lang: str = "amh+eng") -> str:
    ...
    try:
        ...
        # Preprocess for better OCR - enhance contrast and sharpen
        if image.mode != 'RGB':
            image = image.convert('RGB')
        
        # Apply automatic enhancement for better OCR results
        ...
        if len(text.strip()) < 15:
            print(f"DEBUG: Low OCR yield ({len(text)} chars), trying enhanced preprocessing...")
...
def extract_text_from_pdf(pdf_bytes: bytes, lang: str = "amh+eng") -> Tuple[str, Optional[bytes]]:
    ...
    try:
        ...
        # Try enhanced version for better OCR
        enhanced_bytes = enhance_image_for_ocr(img_bytes)
        
        # Extract text from the image
        text = extract_text_from_image(img_bytes, lang)
        
        # If original gave poor results, use enhanced image
        if len(text.strip()) < 15 and enhanced_bytes != img_bytes:
            print(f"DEBUG: PDF OCR yield low ({len(text)}), using enhanced image...")
            text = extract_text_from_image(enhanced_bytes, lang)
            img_bytes = enhanced_bytes  # Use enhanced for AI fallback too
...
def extract_text_from_document(file_content: bytes, file_ext: str, lang: str = "amh+eng") -> Tuple[str, List[str], Optional[bytes]]:
    """Extract text from document (PDF or image) and return enhanced image for AI fallback"""
    notes = []
    text = ""
    processed_image = None
    
    if file_ext in ("jpg", "jpeg", "png", "gif", "webp", "tiff", "bmp"):
        notes.append("Processing image with OCR (with automatic enhancement)")
        
        # Apply enhancement before OCR
        enhanced_content = enhance_image_for_ocr(file_content)
        text = extract_text_from_image(enhanced_content, lang)
        processed_image = enhanced_content  # Use enhanced for AI analysis
        
        if text and len(text) > 10:
            notes.append(f"OCR extracted {len(text)} characters")
        else:
            notes.append("OCR produced limited text - AI vision will analyze the enhanced image")
    
    elif file_ext == "pdf":
        notes.append("Converting PDF to image for OCR (with enhancement)")
...
- Extract: Full Name (Amharic + English), Date of Birth, Issue Date, Expiry Date,FAN
...
- Fiscal Year: Must be clearly stated"""
    ...
    # Professional Photo check (4x3 size photo)
    if "photo" in ll or "profile" in ll or "portrait" in ll:
        rules = """## Professional Photo Verification Rules
- Document: 4x3 size professional portrait photo
- Check: Must be a real photo (not a scan of a document or digital screenshot)
- Check: Must be a single person only (single photo)
- Check: Must be front-facing with clear facial features (frontface)
- Check: Background should be neutral or professional
- Validation: High image quality, no filters, no sunglasses/hats (unless for religious reasons)"""
        
        entities = """{
            "isRealPhoto": {"type": "boolean", "required": true},
            "isSinglePerson": {"type": "boolean", "required": true},
            "isFrontFace": {"type": "boolean", "required": true},
            "hasClearFace": {"type": "boolean", "required": true},
            "isProfessionalQuality": {"type": "boolean"},
...
- Check: Official manufacturer's documentation preferred"""
    ...
    if "logo" in ll or "photo" in ll or "image" in ll or "picture" in ll:
        # Check if it's a representative portrait photo or a company logo
        if any(x in ll for x in ["representative", "person", "profile", "portrait", "professional"]):
            rules = """## Representative Photo Verification Rules
- Document: 4x3 size professional portrait photo of company representative
- Check: Must be a real photo (not a scan of a document or digital screenshot)
- Check: Must be a single person only (single photo)
- Check: Must be front-facing with clear facial features (frontface)
- Check: Background should be neutral or professional
...
- Type: Government ID (National ID)
- Extract: Full Name (English), Date of Birth,FAN
...
- Validate: License must be active (not expired)"""
        ...
        entities = """{
            "LICENSE NO": {"type": "number", "required": true},
            "professionalTitle": {"type": "string", "required": true},
            "professionalGrade": {"type": "string"},
            "issueDate": {"type": "date", "required": true},
            "expiryDate": {"type": "date", "required": true},
            "license Type": {"type": "string", "required": true},
            "issuingAuthority": {"type": "string", "required": true},
            "hasQRCode": {"type": "boolean"},
            "confidence": {"type": "number"},
...
def ai_score_and_details_structured(extracted_text: str, label: str, category_name: str, category_rules: str, extracted_schema: str, image_data: Optional[bytes] = None, file_ext: str = "") -> Tuple[str, str, Optional[str]]:
    ...
    prompt = f"""You are an expert document auditor specializing in Ethiopian government documents. Your task is to verify the authenticity and correctness of documents.

DOCUMENT TO VERIFY: Expected type is '{label}' (Category: '{category_name}')

CRITICAL RULES FOR VERIFICATION:
{category_rules}

OCR-EXTRACTED TEXT (may be incomplete due to document quality):
---
{extracted_text or "(No OCR text available - rely solely on visual analysis of the image)"}
---

IMAGE ANALYSIS INSTRUCTIONS:
- The provided image has been PREPROCESSED to enhance quality (contrast, sharpening, noise reduction applied)
- Even if the image appears blurry, look for these authenticity markers:
  * Government seals and stamps (official embossed or printed)
  * Consistent fonts (no sign of digital editing)
  * Overlapping elements (stamps over text, signatures over stamps)
  * Official paper texture/watermarks
  * QR codes and security features
- If you CAN see enough visual elements to make a determination, do so confidently
- Do NOT return "INCONCLUSIVE" just because text is hard to read - use your visual analysis skills

DOCUMENT TYPE MATCHING (CRITICAL):
- Your FIRST task is to determine if this document is actually a '{label}'
- If the document type does NOT match (e.g., an experience letter is uploaded instead of a National ID), set finalRecommendation to "REJECTED" immediately
- When in doubt about type matching, err on the side of rejecting mismatched documents

VERDICT INSTRUCTIONS:
- "APPROVED": Document is a genuine '{label}', meets all rules, and shows no suspicious patterns
- "REJECTED": Document is fake, tampered, wrong type, or fails critical validation rules
- "INCONCLUSIVE": ONLY use this if you genuinely cannot determine authenticity even from visual analysis (e.g., image is completely unreadable, not just low quality)

EXPECTED JSON OUTPUT FORMAT:
{{
  "summary": "Clear 2-3 sentence summary: What document is this? Is it genuine? What are the key findings?",
  "finalRecommendation": "APPROVED" or "REJECTED" or "INCONCLUSIVE",
  "confidenceScore": 0.0 to 1.0 (float),
  "extractedEntities": {{ "fieldName": "extractedValue", ... }},
  "domainValidation": {{
    "checks": [
      {{ "rule": "Document Type Match", "passed": true/false, "message": "Confirm if this is a {label}" }},
      {{ "rule": "Rule name", "passed": true/false, "message": "Result" }}
    ]
  }},
  "authenticityIndicators": {{
    "suspiciousPatterns": ["List specific suspicious patterns found, or empty array"],
    "overallAuthenticityScore": 0.0 to 1.0,
    "visualClues": ["Key visual elements found (stamps, seals, signatures, etc.)"]
  }},
  "qualityAssessment": {{
    "imageQuality": "good" or "fair" or "poor",
    "ocrQuality": "good" or "poor" or "not_available",
    "issues": ["Any specific issues, or empty array"]
  }}
}}

CRITICAL: 'extractedEntities' must be a FLAT JSON object with field names as keys.
Do NOT include nested type definitions or schema in the output.
