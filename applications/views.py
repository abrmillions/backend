    ...
    def approve(self, request, pk=None):
        ...
        try:
            ...
            if sys_settings.verified_only_true:
                unverified_docs = app.documents.filter(is_verified=False)
                if unverified_docs.exists() or not app.documents.exists():
                    return response.Response(
                        {"detail": "Cannot approve application: Some documents are not verified (verifiedOnlyTrue is ON)."}, 
