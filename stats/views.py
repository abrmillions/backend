...
def admin_analytics_view(request):
    ...
    try:
        ...
        # Consider both 'success' and 'active' (case-insensitive)
        paid_payments = Payment.objects.filter(
            models.Q(status__iexact='success') | 
            models.Q(status__iexact='active') |
            models.Q(status__iexact='paid')
        )
        
        # Calculate total revenue across all currencies (case-insensitive ETB)
        total_revenue = paid_payments.filter(currency__iexact='ETB').aggregate(Sum('amount'))['amount__sum'] or 0
        
        # Monthly revenue trends for the selected range
        monthly_revenue = paid_payments.filter(created_at__gte=start_date, currency__iexact='ETB')\
            .annotate(period=trunc_func('created_at'))\
            .values('period')\
